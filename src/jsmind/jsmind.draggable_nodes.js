define(function(require, exports, module) {
	...
	var jsMind = require('jsmind/jsmind');
	...
	var draggable_nodes = function(jm) {
		this.jm = jm;
		this.e_canvas = null;
			...
			for (var nodeid in nodes) {
				...
				if (node.isroot || node.direction == direct) {
					...
					if (direct == jsMind.direction.right) {
						...
						np = {
							x: nl.x + ns.w - options.line_width,
							y: nl.y + ns.h / 2
						};
						sp = {
							x: sx + options.line_width,
							y: sy + sh / 2
						};
					} else {
						if (nl.x - sx - sw <= 0) {
						...
						np = {
							x: nl.x + options.line_width,
							y: nl.y + ns.h / 2
						};
						sp = {
							x: sx + sw - options.line_width,
							y: sy + sh / 2
						};
					}
    ...
    jsMind.draggable = draggable_nodes;
